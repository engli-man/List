...
void ASSERTS() {
  ...
  {
    auto test = std::is_same_v<List<int>::allocator_type, std::allocator<int>>;
    EXPECT_TRUE(test);
  }
}
...
void CONSTRUCTOR() {
...

  {
    ...
    // std::cout << MemoryManager::allocator_destroyed << "\n";
...
void BASIC_FUNC() {
  std::cout << "Checking basic functions: \n";
  SetupTest();
  List<int> lst;
...
void EXCEPTS() {
  ...
  try {
    ...
    // Actually it may not be 8(although de facto it is),
    //     but the only thing we can demand here is the abscence of memory leaks
